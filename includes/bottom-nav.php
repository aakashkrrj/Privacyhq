...
<nav class="fixed bottom-0 left-0 right-0 w-full z-50 bg-surface shadow-[0px_-2px_4px_rgba(0,0,0,0.04)] flex justify-around items-center h-16 px-2 border-t border-outline-variant/30">
    ...
    <!-- Consent -->
    <?php if (has_permission('manage_consents')): ?>
    ...
    <!-- Requests -->
    <?php if (has_permission('manage_dsr')): ?>
        <a href="index.php?page=data-requests" class="flex flex-col items-center justify-center px-5 py-1.5 rounded-xl no-underline transition-all duration-200 <?php echo in_array($currentPage, ['data-requests', 'dsr-management']) ? 'text-primary bg-secondary-fixed font-semibold' : 'text-on-surface-variant hover:text-primary'; ?>">
    ...
    <!-- Assess -->
    <?php if (has_permission('manage_assessments')): ?>
