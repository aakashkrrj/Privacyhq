...
<div class="space-y-lg">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-md">
        <div class="p-md bg-surface rounded-xl border border-outline-variant shadow-sm">
            <span class="text-caption font-semibold uppercase tracking-wider text-on-surface-variant">Total Assigned</span>
            <div class="mt-base text-display font-bold text-on-surface"><?= $total ?></div>
        </div>
        <div class="p-md bg-surface rounded-xl border border-outline-variant shadow-sm">
            <span class="text-caption font-semibold uppercase tracking-wider text-amber-600">In Progress</span>
    ...
    <div class="bg-surface rounded-xl border border-outline-variant shadow-sm overflow-hidden">
        <div class="p-md border-b border-outline-variant bg-surface-container-low">
            <h2 class="font-semibold text-on-surface text-title-md">Assigned DPIAs</h2>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-surface-container-low text-on-surface-variant font-semibold text-label-md uppercase border-b border-outline-variant">
                        <th class="p-md">ID</th>
                        <th class="p-md">Assessment Name</th>
                ...
                <tbody class="divide-y divide-outline-variant text-body-md text-on-surface">
                    <?php if (empty($my_assessments)): ?>
                        <tr>
                            <td colspan="7" class="p-lg text-center text-on-surface-variant">You have no privacy impact assessments assigned to you.</td>
                        </tr>
                    <?php else: ?>
                            ...
                            <tr class="hover:bg-surface-container-lowest transition-colors">
                                ...
                                <td class="p-md text-right">
                                    <?php if ($item['status'] !== 'Approved'): ?>
                                        <a href="index.php?page=perform-assessment&id=<?= $item['id'] ?>" class="text-primary hover:underline font-semibold">Perform</a>
                                    <?php else: ?>
                                        <span class="text-emerald-600 font-semibold flex items-center justify-end gap-xs"><span class="material-symbols-outlined text-[16px]">check_circle</span> Approved</span>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
